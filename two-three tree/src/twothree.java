...
public class twothree {
   ...
   for (int i =0; i<outputCount; i++){
	   ...
	   listOutput(myTree.root, first, second);
   }
   }
   
   static void listOutput(Node rootNode, String start, String end){
	  if(start.compareTo(end)<=0){
	  if ( rootNode instanceof LeafNode) {
		  
		  if(rootNode.guide.compareTo(start)<0 || rootNode.guide.compareTo(end)>0){
			  return;
		  }
		  LeafNode newNode = new LeafNode();
		  newNode = (LeafNode) rootNode;
		  System.out.println(newNode.guide + " " + newNode.value);
		  return;}
	  
	  else { InternalNode newINode = new InternalNode();
	  	newINode = (InternalNode)rootNode;
		  if(newINode.child0.guide.compareTo(start)>=0)listOutput(newINode.child0, start, end);
		  if(newINode.child0.guide.compareTo(end)<=0)listOutput(newINode.child1, start, end);
		  if(newINode.child2!=null && newINode.child1.guide.compareTo(end)<=0)listOutput(newINode.child2, start, end);
		  return;
	  }
	  }
	  else
		  listOutput(rootNode,  end, start);
}
}
